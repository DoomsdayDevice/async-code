...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            try {
                const updatedTasks = await Promise.all(runningTasks.map((task) => ApiService.getTaskStatus(user.id!, task.id)));
    ...
    const getAgentIcon = (agent: string) => {
        switch (agent) {
            case "claude":
                return <ClaudeIcon className="w-3 h-3" />;
            case "codex":
                return <OpenAIIcon className="w-3 h-3" />;
            default:
                return null;
        }
    };
                                ...
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <Avatar className="cursor-pointer">
                                            <AvatarFallback>{user?.email ? user.email.split("@")[0].slice(0, 2).toUpperCase() : "U"}</AvatarFallback>
                                        </Avatar>
                                    </DropdownMenuTrigger>
                            ...
                            <Card>
                                <CardHeader>
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="text-sm text-slate-500">{tasks.length} tasks loaded</div>
                                    </div>
                                    <CardDescription>Track all your automation tasks</CardDescription>
                                </CardHeader>
                                        ...
                                        <>
                                            ...
                                            {hasMoreTasks && (
                                                <div className="flex justify-center pt-4">
                                                    <Button onClick={loadMoreTasks} disabled={isLoadingMore} variant="outline" className="gap-2">
                                                        {isLoadingMore ? (
                                                            <>
                                                                <div className="animate-spin">
